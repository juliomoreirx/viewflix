...
function calcularPreco(minutos) {
  ...
  // Calcula o preço de forma mais precisa
  // Evita problemas de ponto flutuante
  const precoExato = (PRECO_POR_HORA * minutos) / 60;
  
  // Arredonda para o centavo mais próximo (mais justo que Math.ceil)
...
async function verificarBloqueio(userId) {
  try {
    ...
    if (user && user.isBlocked) {
      return {
        blocked: true,
        reason: user.blockedReason || 'Sua conta foi bloqueada pelo administrador.'
      };
    }
...
async function salvarConteudoComprado(userId, videoId, mediaType, title, price, episodeName = null, season = null) {
  try {
    ...
    // ===== DIFERENCIAÇÃO DE EXPIRAÇÃO =====
    // Filmes: 24 horas
    // Séries: 7 dias
    const horasExpiracao = mediaType === 'movie' ? 24 : (7 * 24); // 24h ou 168h
...
async function listarPorLetra(chatId, tipo, letra, pagina = 1) {
  try {
    ...
    if (vouverService.CACHE_CONTEUDO.series.length === 0) {
    ...
    if (letra === '#') {
      ...
      resultados = lista.filter(i => 
        !isAdulto(i.name) && 
        i.name.toUpperCase().startsWith(letra)
      );
    }
    ...
    if (pagina > 1) {
      navRow.push({
        text: '◀️ Anterior',
        callback_data: `letter_${tipo}_${letra}_${pagina - 1}`
      });
    }
...
async function mostrarDetalhesConteudo(chatId, contentId) {
  try {
    ...
    if (!content) {
      bot.sendMessage(chatId, '❌ Conteúdo não encontrado.');
      return;
    }
    ...
    const mensagem = 
      `${emoji} *${content.title}*\n\n` +
      (content.episodeName ? `📺 ${content.episodeName}\n\n` : '') +
...
async function verificarConteudosExpirando() {
  try {
    ...
    // Filmes: notifica 2h antes
    // Séries: notifica 1 dia antes (24h)
    
    // Busca filmes expirando em 2h
  ...
  try {
    ...
    if (isNew) {
      welcome = 
        ...
        `⏱️ Ou, se preferir, pague apenas R$ 2,50 por hora assistida\n\n` +
...
bot.onText(/\/ajuda|\/help/, (msg) => {
  ...
  const helpText = 
    ...
    `💡 *Dica:* Séries têm 7 dias para assistir!\n\n` +
    `*Dúvidas?* Entre em contato com nosso suporte!`;
  
  bot.sendMessage(chatId, helpText, {
    parse_mode: 'Markdown',
    reply_markup: {
      inline_keyboard: [[{ text: '🏠 Menu Principal', callback_data: 'back_main' }]]
    }
  });
});
...
bot.onText(/\/notificacoes|\/notifications/, async (msg) => {
  ...
  try {
    ...
    if (!user) {
      bot.sendMessage(chatId, '❌ Usuário não encontrado. Use /start primeiro.');
      return;
    }
    ...
    bot.sendMessage(chatId,
      ...
      `Deseja ${user.notificationsEnabled ? 'desativar' : 'ativar'}?`,
      {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ 
              text: user.notificationsEnabled ? '❌ Desativar Notificações' : '✅ Ativar Notificações',
...
bot.onText(/\/admin_users/, async (msg) => {
  ...
  try {
    ...
    const recentUsers = await User.find()
      .sort({ registeredAt: -1 })
      .limit(10)
      .select('userId firstName username registeredAt credits totalSpent');
    ...
    recentUsers.forEach((user, index) => {
      message += 
        `${index + 1}. ${user.firstName}${user.username ? ` (@${user.username})` : ''}\n` +
        `   ID: \`${user.userId}\`\n` +
...
bot.onText(/\/admin_user (.+)/, async (msg, match) => {
  ...
  try {
    ...
    if (!user) {
      bot.sendMessage(chatId, `❌ Usuário ID \`${searchId}\` não encontrado.`, { parse_mode: 'Markdown' });
      return;
    }
    
    const conteudosAtivos = await PurchasedContent.countDocuments({
      userId: searchId,
      expiresAt: { $gt: new Date() }
    });
...
bot.onText(/\/admin_block (\d+) (.+)/, async (msg, match) => {
  ...
  try {
    ...
    if (!user) {
      bot.sendMessage(chatId, `❌ Usuário ID \`${targetUserId}\` não encontrado.`, { parse_mode: 'Markdown' });
      return;
    }
...
bot.onText(/\/admin_stats/, async (msg) => {
  ...
  try {
    ...
    const totalCompras = await User.aggregate([
      { $group: { _id: null, total: { $sum: '$totalPurchases' } } }
    ]);
    
    const totalReceita = await User.aggregate([
    ...
    const conteudosExpirados = await PurchasedContent.countDocuments({
    ...
    const message = 
      ...
      `📦 *Conteúdos*\n` +
      `• Ativos (não expirados): ${conteudosAtivos}\n` +
...
bot.onText(/🎬 Filmes A-Z/, (msg) => {
  const chatId = msg.chat.id;
  mostrarAlfabeto(chatId, 'movies');
});

bot.onText(/📺 Séries A-Z/, (msg) => {
  const chatId = msg.chat.id;
  mostrarAlfabeto(chatId, 'series');
});

bot.onText(/📦 Meu Conteúdo/, async (msg) => {
  const chatId = msg.chat.id;
...
bot.onText(/💰 Adicionar Créditos/, (msg) => {
  const chatId = msg.chat.id;
...
bot.on('message', async (msg) => {
  ...
  if (!state || !['search_movies', 'search_series', 'search_adult'].includes(state.step)) {
    return;
  }
  ...
  try {
    ...
    if (state.step === 'search_adult') {
      ...
      resultados = todosItens
        .filter(i => isAdulto(i.name) && i.name.toLowerCase().includes(termo))
        .slice(0, 15);
    } else {
  ...
  try {
    ...
    if (data.startsWith('add_')) {
      ...
      if (isNaN(valor) || valor <= 0) {
        bot.answerCallbackQuery(query.id, { text: 'Valor inválido' });
        return;
      }
      ...
      if (pix.pix_qr_base64) {
        await bot.sendPhoto(chatId, Buffer.from(pix.pix_qr_base64, 'base64'), {
          caption: '📱 Ou escaneie este QR Code com seu app de banco'
        });
      }
    ...
    if (data.startsWith('details_')) {
      ...
      if (!detalhes) {
        bot.sendMessage(chatId,
          '❌ Erro ao carregar detalhes do conteúdo.',
          {
            reply_markup: {
              inline_keyboard: [[{ text: '🏠 Menu Principal', callback_data: 'back_main' }]]
            }
      ...
      let mensagem = `🎬 *${detalhes.title}*\n\n`;
      
      if (detalhes.info.ano || detalhes.info.genero || detalhes.info.imdb) {
        if (detalhes.info.genero) mensagem += `🎭 ${detalhes.info.genero}\n`;
      ...
      if (detalhes.info.sinopse) {
        const sinopse = detalhes.info.sinopse.substring(0, 400);
      ...
      if (detalhes.mediaType === 'movie') {
        // ===== FILMES: USA DURAÇÃO DO HTML (SEMPRE!) =====
        let minutos = 109;
        ...
        if (detalhes.info.duracaoTexto) {
          mensagem += `⏱️ Duração: ${detalhes.info.duracaoTexto} (${minutos}min)\n`;
        } else {
          mensagem += `⏱️ Duração: ~${duracaoTexto}\n`;
        }
        
        mensagem += `💰 Preço: ${formatMoney(preco)} (R$ 0,04/min)\n`;
        mensagem += `⏰ Válido por: 24 horas\n`;
        mensagem += `💳 Seu saldo: ${formatMoney(saldoAtual)}`;
        ...
        if (saldoAtual < preco) {
          ...
          keyboard.push([{
            text: `▶️ Assistir - ${formatMoney(preco)}`,
            callback_data: `watch_movie_${id}_${preco}_${minutos}`
          }]);
        }
      } else {
        mensagem += `📺 *Temporadas disponíveis:*\n\n`;
        mensagem += `⏰ Válido por: 7 dias\n`;
        mensagem += `💳 Seu saldo: ${formatMoney(saldoAtual)}\n\n`;
        
        Object.keys(detalhes.seasons).forEach(season => {
          const numEps = detalhes.seasons[season].length;
          keyboard.push([{
            text: `Temporada ${season} (${numEps} episódio${numEps > 1 ? 's' : ''})`,
            callback_data: `season_${id}_${season}`
          }]);
        });
      }
    ...
    if (data.startsWith('season_')) {
      ...
      for (let i = 0; i < episodios.length; i++) {
        ...
        keyboard.push([{
          text: `${i + 1}. ${ep.name.substring(0, 50)}${ep.name.length > 50 ? '...' : ''}`,
      ...
      keyboard.push([{
        ...
        callback_data: `details_${id}_${state.type}`
      }]);
    ...
    if (data.startsWith('episode_')) {
      ...
      let mensagem = 
        `📺 *${state.data.title}*\n` +
        `*Temporada ${season} - ${episodio.name}*\n\n` +
      ...
      if (saldoAtual < preco) {
        ...
        keyboard.push([{
          text: `▶️ Assistir - ${formatMoney(preco)}`,
          callback_data: `watch_ep_${epId}_${preco}_${season}`
        }]);
      }
      
      keyboard.push([{
        text: '⬅️ Voltar à Temporada',
        callback_data: `season_${state.id}_${season}`
      }]);
    ...
    if (data.startsWith('watch_movie_')) {
      ...
      const id = parts[2];
      const preco = parts[3];
      const minutosString = parts[4];
      
      const precoNum = parseInt(preco);
      const minutosReais = minutosString ? parseInt(minutosString) : 109;
      ...
      const token = await salvarConteudoComprado(
        chatId,
        id,
        'movie',
        state.data.title,
      ...
      await enviarVideoComLink(
        chatId,
        token,
        `🎬 ${state.data.title} (${minutosReais}min)`,
        precoNum,
        state.data.title,
        'movie'
      );
    ...
    bot.sendMessage(chatId, '❌ Erro ao processar sua solicitação. Tente novamente.');
  }
});
...
bot.on('polling_error', (error) => {
  console.error('Erro de polling do bot:', error);
});
...
// Verifica notificações 30s após iniciar
